...
def process_message(ch, method, properties, body):
    try:
        ...
        if model_name == "local1":
            logger.info("Processing message for Local Model 1")
            ...
            rabbitmq.publish_message(
                "chatroom_history", "sports.football.local1", local1_response
            )
        elif model_name == "local2":
            logger.info("Processing message for Local Model 2")
            ...
            rabbitmq.publish_message(
                "chatroom_history", "sports.football.local2", local2_response
            )
        elif model_name == "haiku":
            logger.info("Processing message for Claude-haiku")
            ...
            rabbitmq.publish_message(
                "chatroom_history", "sports.football.haiku", claude_response
            )
...
def main():
    ...
    rabbitmq.channel.queue_bind(
        exchange="llm_communication",
        queue="sports.football.haiku",
        routing_key="sports.football.haiku",
    )
    rabbitmq.channel.queue_bind(
        exchange="llm_communication",
        queue="sports.football.local1",
        routing_key="sports.football.local1",
    )
    rabbitmq.channel.queue_bind(
        exchange="llm_communication",
        queue="sports.football.local2",
        routing_key="sports.football.local2",
    )
    ...
    for model_name in ["local1", "local2", "haiku"]:
        ...
        rabbitmq.publish_message(
            "llm_communication", f"sports.football.{model_name}", message
        )
        time.sleep(delay)
...
def create_queue_name(topic: str, subtopic: str, model_name: str) -> str:
    """Creates a queue name string based on topic, subtopic, and model name."""
    return f"{topic}.{subtopic}.{model_name}"
